/*
 * Compile : g++ -o Smooth Smooth.cpp -lpthread
 * Execute : ./Smooth <number of thread>
 * Create by siahuat0727
 */
...
void* thread_func(void* id){
	...
	for(int count = 0; count < NSmooth ; count ++) {
		// print out something
		if(thread_id == 0 && (count + 1) % 50 == 0)
			cout << "Smoothing: " << count+1 << "/" << NSmooth << endl;
...
int main(int argc,char *argv[])
{
	...
	if(argc != 2)
		error("Invalid number of parameter. (./Smooth <number of thread>)");
	...
	// store end time
	long long int terminate = time(0);
	...
	//得到結束時間，並印出執行時間
	cout << "The execution time = "<< terminate - start << " seconds" << endl ;
